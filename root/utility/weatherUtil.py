...
class GetWeatherApp(threading.Thread):
    ...
    def checkWarningTextCache(self):
        ...
        if mtime == None or ((datetime.datetime.now() - mtime).total_seconds() > 3600):
            warningText = requestHTML("https://www.hko.gov.hk/wxinfo/dailywx/wxwarntoday_uc.htm", self.getWarningScrape)
    ...
    def checkWeatherResultDictCache(self):
        ...
        try:
            ...
            pass
        if mtime == None or ((datetime.datetime.now() - mtime).total_seconds() > 3600):
            weatherResultDict = requestHTML("https://www.hko.gov.hk/contentc.htm", self.getWeatherScrape)
